...
from .models import IssuedLicense, LicenseRequest, Machine, Product, ProductRelease, User
...
def register_user_routes(templates: Jinja2Templates) -> APIRouter:
    ...
    def create_request(
        ...
        db.add(request_row)
        db.commit()
        push_flash(request, 'success', 'Access request submitted.')
        return RedirectResponse('/app', status_code=303)
    ...
    def download_installer(release_id: int, request: Request, db: Session = Depends(get_db)):
        ...
        has_access = (
            db.query(IssuedLicense)
            .filter(IssuedLicense.user_id == current_user.id, IssuedLicense.product_id == release.product_id)
            .first()
        )
